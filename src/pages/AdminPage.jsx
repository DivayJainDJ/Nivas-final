      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px] xl:items-end">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {[
            ...
            { icon: Wand2, label: 'City briefing' },
...
function ComplaintsAnalytics({ data }) {
  return (
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
      <div data-admin-reveal className="admin-panel rounded-[28px] p-5">
        <SectionTitle icon={Activity} eyebrow="Complaints intelligence" title="Resolution timeline and unresolved pressure" />
...
function HousingAnalytics({ data }) {
  return (
    <section className="grid gap-5 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
      <div data-admin-reveal className="admin-panel rounded-[28px] p-5">
        <SectionTitle icon={Home} eyebrow="Housing allocation" title="Inventory, waitlist, and readiness" />
    ...
    <section data-admin-reveal className="admin-panel rounded-[28px] p-5">
        ...
        action={
          <div className="flex flex-wrap gap-2">
            {['Open reports', 'Compare scans', 'Export summaries', 'Map overlays'].map((label) => (
              <button key={label} type="button" className="admin-small-button">{label}</button>
            ))}
          </div>
        }
      />
      <div className="mt-5 grid gap-3 xl:grid-cols-5">
        {data.analysisHistory.map((record) => (
          <article key={record.id} className="rounded-[22px] border border-product-line bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
      ...
      <div data-admin-reveal className="admin-panel rounded-[28px] p-5">
        ...
        <div className="mt-4 space-y-3">
          {[
            ...
            <button key={title} type="button" className="w-full rounded-2xl border border-product-line bg-white p-4 text-left transition hover:-translate-y-0.5 hover:shadow-soft">
