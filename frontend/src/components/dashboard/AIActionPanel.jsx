...
export default function AIActionPanel({ actions, wards }) {
  const { goToSlumPlanner, goToHousingMatch, goToOfficer } = useAppNavigation('dashboard-ai-recommendation')
  ...
  const openAction = (action) => {
    ...
    if (text.includes('housing') || text.includes('ews') || text.includes('waitlist')) {
      goToHousingMatch(null, { state: { actionId: action.id, wardId: action.wardId } })
      return
    }
    if (text.includes('response team') || text.includes('crew')) {
      goToOfficer({ state: { actionId: action.id, wardId: action.wardId } })
      return
    }
    goToSlumPlanner(action.wardId, { state: { actionId: action.id, wardId: action.wardId } })
  }
