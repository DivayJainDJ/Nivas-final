...
const chips = [
  { label: 'Ward overview', to: ROUTES.DASHBOARD },
  { label: 'Complaints', to: ROUTES.COMPLAINTS },
  { label: 'Housing match', to: ROUTES.HOUSING_MATCH },
  { label: 'Slum planner', to: ROUTES.SLUM_PLANNER },
  { label: 'Admin analytics', to: ROUTES.ADMIN },
  { label: 'Demo mode', to: ROUTES.DEMO },
]
    ...
    <section className="landing-hero relative overflow-hidden px-4 pb-20 pt-36 md:px-6 md:pb-24 md:pt-40">
      ...
      <div className="relative z-10 mx-auto mt-16 max-w-7xl">
        <div className="landing-reveal landing-delay-4 flex flex-wrap items-center gap-2 rounded-[28px] border border-white/[0.78] bg-white/[0.64] p-3 shadow-soft backdrop-blur-xl">
          ...
          {chips.map((chip) => (
            <Link key={chip.label} to={chip.to} className="rounded-full border border-product-line bg-white px-4 py-2 text-sm font-extrabold text-product-slate shadow-sm transition hover:-translate-y-0.5 hover:text-product-navy">
              {chip.label}
            </Link>
          ))}
        </div>
